...
if "stocks_processed_with_current_key" not in st.session_state:
    st.session_state["stocks_processed_with_current_key"] = 0
if "stocks_per_key" not in st.session_state:
    st.session_state["stocks_per_key"] = 6
if "processed_symbols" not in st.session_state:
    st.session_state["processed_symbols"] = set()
...
with col1:
    st.session_state["stocks_per_key"] = st.number_input(
        "Stocks to process per key",
        min_value=1,
        value=st.session_state["stocks_per_key"],
        help="Number of stocks to process before switching to the next API key"
    )

# Parse the keys
if api_keys_input:
    st.session_state["api_keys"] = [key.strip() for key in api_keys_input.split('\n') if key.strip()]
    total_capacity = len(st.session_state["api_keys"]) * st.session_state["stocks_per_key"]
    st.write(f"Found {len(st.session_state['api_keys'])} API keys. Can process approximately {total_capacity} stocks.")
...
with col1:
    if st.button("Fetch Articles"):
        if not get_current_api_key().strip():
            ...
            for symbol in symbols:
                st.session_state["process_status"].append(f"Fetching articles for: {symbol} (Using API key {st.session_state['current_key_index'] + 1} of {len(st.session_state['api_keys'])})")
...
with col2:
    ...
    if st.button("Fetch Content", disabled=not st.session_state["articles_fetched"]):
        ...
        key_status.text(f"Using API key {st.session_state['current_key_index'] + 1} of {len(st.session_state['api_keys'])} | " +
                        f"Processed {st.session_state['stocks_processed_with_current_key']} of {st.session_state['stocks_per_key']} stocks with current key")
        
        for csv_file in csv_files:
            symbol = csv_file.replace("_news_data.csv", "")
            
            # Check if we need to rotate the key
            if symbol not in st.session_state["processed_symbols"] and st.session_state["stocks_processed_with_current_key"] >= st.session_state["stocks_per_key"]:
                rotate_to_next_key()
                # Update key status display
                key_status.text(f"Using API key {st.session_state['current_key_index'] + 1} of {len(st.session_state['api_keys'])} | " +
                               f"Processed {st.session_state['stocks_processed_with_current_key']} of {st.session_state['stocks_per_key']} stocks with current key")
            
            st.session_state["process_status"].append(f"Fetching summaries for {symbol} (Using API key {st.session_state['current_key_index'] + 1} of {len(st.session_state['api_keys'])})")
            ...
            # Mark this symbol as processed and increment the counter
            if symbol not in st.session_state["processed_symbols"]:
                st.session_state["processed_symbols"].add(symbol)
                st.session_state["stocks_processed_with_current_key"] += 1
                # Update key status display
                key_status.text(f"Using API key {st.session_state['current_key_index'] + 1} of {len(st.session_state['api_keys'])} | " +
                               f"Processed {st.session_state['stocks_processed_with_current_key']} of {st.session_state['stocks_per_key']} stocks with current key")
